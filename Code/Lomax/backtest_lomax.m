function [backtest_window,exceptions,VaR] = backtest_lomax(data,alpha,...
    window_start,window_end,N,mode)
% BACKTEST_LOMAX  VaR backtest over an out-of-sample window (fixed or rolling)
% for the Comb-Bernoulli copula model with Lomax marginals.
%
%   Calibrates the single claim model on a training window, simulates N loss
...
%     exceptions      : exception flags, true where the realized Total exceeds
%                       VaR.
%                       'Fixed'          -> M x nAlpha;
%                       'Rolling-window' -> 1 x nAlpha x M (3rd dim = day).
%     VaR             : Value-at-Risk estimates for the single model.
%                       'Fixed'          -> 1 x nAlpha;
%                       'Rolling-window' -> 1 x nAlpha x M (3rd dim = day).
%
%   NOTE:
%     - nAlpha = numel(alpha); M = height(backtest_window).
%     - VaR and exceptions change shape between modes: any consumer must
%       therefore branch on mode (see plot_backtest_lomax).
...
if strcmp(mode,'Fixed')
    ...
    VaR = var_calc_lomax(data,alpha,window_start,...
        window_end,N);

 
    exceptions = (backtest_window.Total > VaR);
...
elseif strcmp(mode,'Rolling-window')
    ...
    VaR = zeros(1,2,size(backtest_window,1));
    exceptions = zeros(1,2,size(backtest_window,1));
    ...
    for i = 1:size(backtest_window,1)
        ...
        VaR(:,:,i) = var_calc_lomax(data,alpha,window_start,...
            window_end,N);

        j = j+1;


        exceptions(:,:,i) = (backtest_window.Total(i) > VaR(:,:,i));
