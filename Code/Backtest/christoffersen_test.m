...
%
%   OUTPUT
%     out        : 1xM struct; out(m).level(L) holds the counts, LR
%                  statistics and p-values for model m at level L.

%% Tests
levels    = [0.05, 0.01];
lvl_names = {'95%','99%'};
M         = numel(exceptions); % How many models
out       = struct();

for m = 1:M
    for i = 1:2
        out(m).level(i) = single_test(double(exceptions{m}(:,i)), ...
